...
public class Databases {
    ...
    public static boolean init() {
        ...
        if (!Databases.getMySQL().init()) {
            AdvancedPet.getInstance().getLogger().severe("[Database] Can't initialize MySQL.");
            AdvancedPet.getInstance().getLogger().severe("[Database] Will be using YAML support instead.");
        ...
        try {
            while (playerData.next()) {
                ...
                for(String seriaPetName : namesTable)
                {
                    ...
                    {
                        ex.printStackTrace();
                        AdvancedPet.getInstance().getLogger().severe("[Database] Index out of bound for (65) : " + seriaPetName);
                    }
                }
